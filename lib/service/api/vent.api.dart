      ...
      if (response.statusCode == 200 && res.success) {
        ...
        ventController.vents.forEach((vent) {
          if (vent.id == ventController.selectedVent.value!.id) {
            ...
            if (!vent.commentList.map((x) => x.id).contains(vComment.id)) {
              vent.commentList.add(vComment);
              vent.comments++;
            }
            ventController.selectedVent.value = null;
            ventController.selectedVent.value = vent;
      ...
      if (response.statusCode == 200 && res.success) {
        for (int i = 0; i < ventController.vents.length; i++) {
          if (ventController.vents[i].id == ventId) {
            ...
            ventController.vents.value[i].likes = ventController.vents.value[i].isLiked ? ventController.vents.value[i].likes - 1 : likeType ? ventController.vents.value[i].likes + 1 : ventController.vents.value[i].likes;
